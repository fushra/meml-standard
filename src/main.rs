// MEML parser is a markup language for the web inspired by Lisp
...
#[derive(Debug)]
pub enum MEML<'a> {
    Tag(&'a str, HashMap<&'a str, &'a str>, Vec<MEML<'a>>),
    Text(&'a str),
}
...
fn main() {
    ...
    if let Some(input) = input {
        ...
        let p = MEMLParser::parse(Rule::root, &input);
...
fn tag(t: Pair<Rule>) -> MEML {
    ...
    let mut ats = if let Some(n) = t.next() {
        ...
        return MEML::Tag(id.as_str(), attributes, vec![]);
    };
    ...
    MEML::Tag(id.as_str(), attributes, body(ats))
}

fn body(b: Pair<Rule>) -> Vec<MEML> {
    // body = ${ (comment | tag | text)+ }
    let mut result = vec![];
    for i in b.into_inner() {
        result.push(match i.as_rule() {
            Rule::tag => tag(i),
            Rule::text => MEML::Text(i.as_str()),
            Rule::comment => continue,
...
// Implement display trait for MEML to allow html creation
impl<'a> fmt::Display for MEML<'a> {
